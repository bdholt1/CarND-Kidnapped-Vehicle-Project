...
int main() {
	...
	int time_steps_before_lock_required = 2443; // number of time steps before accuracy is checked by grader.
	...
	for (int i = 0; i < num_time_steps; ++i) {
		...
		pf.resample();
		ostringstream pf_file;
		pf_file << "data/particles/particles_" << setfill('0') << setw(6) << i+1 << ".txt";
        pf.write(pf_file.str());
